...
static uint8_t l_polynom[16] = {148,32,133,16,194,192,1,251,1,192,194,16,133,32,148,1};
...
namespace blockKuz{
	...
	class LSX{
		public:
			...
			void L (uint8_t *block,const uint8_t targ[16]);
			void inL(uint8_t *block, const uint8_t targ[16]);
			std::vector<uint8_t> polynom = {1,148,32,133,16,194,192,1,251,1,192,194,16,133,32,148};//{148,32,133,16,194,192,1,251,1,192,194,16,133,32,148,1};
			void generate_key(uint8_t *key,uint8_t &order,uint8_t pol[16]);
			LSX();
			~LSX();
		protected:
			uint8_t *newVector;
	...
	void LSX::L(uint8_t *block,const uint8_t targ[16]){
		uint8_t c = 0;
		int8_t j = 0;
		for (uint8_t i = 0; i < 16; i++){
			for(j = 15; j >=0 ; j--)
				c ^= polinMultiple[block[j]][targ[j]];
			for(j = 15; j > 0; j--)
				block[j] = block[j-1];
			block[0] = c;
			c = 0;
	...
	void LSX::inL(uint8_t *block, const uint8_t targ[16]){
		uint8_t c=0, j=0,x=0;				
		for (uint8_t i = 0; i<16; i++){
			c = block[0];
			for (j = 0; j<15; j++)
	...
	void LSX::generate_key(uint8_t *key,uint8_t &order,uint8_t pol[16]){
	...
	void EncDec::makekeys(uint8_t *key){
		...
		for(uint8_t i=0; i <5; i++){
			...
			for(uint8_t j=0; j<i; j++)deffight.generate_key(cop[i],j,l_polynom);
		}
	}
...
int main(int argc, char* argv[]){
	...
	//m.decrypt(open_text);
	for (int i = 0; i<16; i++)
		printf("%02x ", block_text[i]);
	printf("\n");
	//blockKuz::LSX kek;
	//blockKuz::generateMatrix(matrix);
	//kek.L(a,matrix);
	//kek.L(b,l_polynom);
